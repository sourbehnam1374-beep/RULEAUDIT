...
For the philosophical motivation, see the manuscript draft in `archive/manuscript/`. For practical instructions on adding a new rule, see `ADDING_A_RULE.md`. For interpretation of completed audit reports, see `INTERPRETING_REPORTS.md`. For the published literature each test is grounded in — and how RuleAudit relates to TRIPOD, PROBAST, and the derivation/validation/impact lifecycle — see `LITERATURE.md`.
...
RuleAudit is a **data-free, structural pre-check** that sits *before* the
clinical-prediction-rule lifecycle of **derivation → validation → impact
analysis** (McGinn et al., JAMA 2000) — i.e. it runs when an expert rule has been
written down but no derivation cohort yet exists. It is complementary to, not a
replacement for, the data-driven appraisal tools: the **TRIPOD** reporting
checklist (22 items; 2015, TRIPOD+AI 2024) and the **PROBAST** risk-of-bias tool
(four domains — participants, predictors, outcome, analysis; 2019). Tests I–V
anticipate PROBAST's *predictor*-domain concerns on the bare rule, before there
is a fitted model for PROBAST to assess.

Each test draws on an established methodology:

| Test | Grounded in |
|------|-------------|
| I — Firing rates | Information theory: binary-indicator entropy H(p) |
| II — Orthogonality | Collinearity diagnostics (double-counted signal) |
| III — VIF | Multicollinearity / variance inflation; redundant model description length |
| IV — Sensitivity | Variance-based (Sobol') global SA; total-order index S_T (Homma & Saltelli, 1996; Saltelli et al., *Global Sensitivity Analysis: The Primer*, 2008). OAT alone cannot detect interactions — hence the S_T vs S₁ comparison. |
| V — Identifiability | Combinatorial label diversity as a structural correlate of external-validation consistency |
| VI — MDL calibration debt | Two-part Minimum Description Length, L(D;M)=L(M)+L(D\|M) (Rissanen, 1978; Grünwald, 2007); penalty derived from coding theory rather than an ad-hoc heuristic |

The worked examples are chosen to exhibit one failure mode each: **Lewinnek
(1978)** safe zone — *mechanism erasure* (Abdel et al., CORR 2016: 58% of
dislocated THAs sat inside the safe zone); **GAP (Yilgor et al., JBJS 2017)** —
*mechanism sprawl* and *hidden interactions*; **SHIVA SPIN-THA (Sour, 2026)** —
*driver inflation*. Full citations and the verification notes are in
`LITERATURE.md`.
